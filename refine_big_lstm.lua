...
cmd:option('-modelpath', '', 'path to model to refine')
cmd:option('-dicpath', '', 'path to dictionary file')
...
local orig_config = all.config
dic = data.sortthresholddictionary(dic, orig_config.threshold)
orig_model = all['model']
local lut = orig_model.modules[1].modules[2].modules[1]
local rnn = orig_model.modules[2]
local decoder = orig_model.modules[7]
orig_model:cuda()
orig_model:remove()
orig_model:evaluate()
collectgarbage()
...
local cutoff = tablex.map(tonumber, stringx.split(orig_config.cutoff, ','))
